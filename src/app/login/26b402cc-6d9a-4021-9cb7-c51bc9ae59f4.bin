...
function LoginForm() {
  ...
  return (
    <div className="max-w-sm mx-auto px-4 py-16">
      ...
      {justReset && (
        <p className="border-2 border-ink/10 rounded-card px-4 py-3 text-sm text-ink mb-4" role="status">
      ...
      <form onSubmit={handleSubmit} className="space-y-4" noValidate>
        ...
        <div className="text-right -mt-2">
          <Link href="/forgot-password" className="text-sm text-chili font-semibold hover:underline">
            Forgot password?
          </Link>
        </div>
        {error && (
