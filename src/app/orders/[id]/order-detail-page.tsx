    ...
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      {order.status === "PENDING_PAYMENT" && (
        <div className="bg-marigold/10 border-2 border-marigold rounded-card p-4 mb-6">
          <p className="font-semibold text-ink mb-1">This order is awaiting M-Pesa payment.</p>
          {paymentError && <p className="text-chili text-sm mb-3">{paymentError}</p>}
          <button onClick={retryPayment} disabled={busy} className="bg-marigold text-ink font-bold px-4 py-2 rounded-card hover:bg-marigoldDark transition-colors disabled:opacity-60 mt-2">
